...
def count_matrix(features,feature_train):
    feature_name = []
    ...
    if feature_train == None:
        ...
        return sp.csr_matrix((feature_count, feature_name, index_point), shape=(len(index_point) - 1, 33381),
                             dtype=np.int64).toarray(),feature_dict
    else:
        feature_dict=feature_train
        for feature_line in features:
            feature_counter = {}
            for feature in feature_line:
    ...
    return sp.csr_matrix((feature_count, feature_name, index_point), shape=(len(index_point) - 1, 33381), dtype=np.int64).toarray()


def featureExtraction(texts, classes, train_file,feature_level='unigram'):
    ...
    if train_file==None:
        feature_matrix,featureDict = count_matrix(features,None)
        for label in classes:
            class_list.append(class_dict.setdefault(label, len(class_dict)))

        featureWithLabel = list(zip(feature_matrix, class_list))
        return featureWithLabel, class_dict,featureDict
    else:
        feature_matrix = count_matrix(features,train_file)
...
def perform_operations(file_name,testFile):
    ...
    for idx, line in enumerate(testData.split("\n")):
        line_contents = line.split('\t')
        if any(line_contents):
            # task-3. escape 'none' label class rows.
            if line_contents[3].lower() == 'none':
                continue
            # text
            testText.append(line_contents[1])
            # reason
            testClass.append(line_contents[3])
    del data
    global CLASS_NAMES
    global CLASS_DICTIONARY

    labelFeature, CLASS_DICTIONARY,feature_dict = featureExtraction(texts, classes, None, 'all')
    X_train = [l[0] for l in labelFeature]
    y_train = [l[1] for l in labelFeature]

    testFeature, TEST_CLASS_DICTIONARY = featureExtraction(testText, testClass, feature_dict, 'all')
    X_test = [l[0] for l in testFeature]
    y_test = [l[1] for l in testFeature]
    CLASS_NAMES = np.asarray(y_train, dtype=np.int64)
...
def main():
    ...
    if file_name != '':
        perform_operations(file_name,testFile)
    else:
